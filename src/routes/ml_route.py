...
def get_feeling():
    try:
        ...
        modelo_carregado, X_ngrams, df_results_true_count = load_sentiment_model()

        # Converter df_results_true_count para um dicionário
        sentiment_counts_dict = df_results_true_count.to_dict()
...
@blueprint_ml.route('/feeling_test')
def get_feeling_test():
    try:
        modelo_carregado, X_ngrams, df_results_true_count = load_sentiment_model()
